...
class ScholarshipScreen extends React.Component {
  ...
  render() {
    ...
    let shareOptions = {
      title: "The Academist",
      message: "I just found scholarships on The Academist",
      url: `https://www.theacademist.com/register?ref=${referralCode}`,
      subject: "Scholarships on The Academist" //  for email
    };
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    unsaveScholarship: (url, token, user_id, scholarship_id, savedArray) => 
      {
        dispatch(
          scholarshipUnSaveCall(url, token, user_id, scholarship_id, savedArray)
        );
      },
    saveScholarship: (url, token, user_id, scholarship_id, savedArray) => 
      {
        dispatch(
          scholarshipSaveCall(url, token, user_id, scholarship_id, savedArray)
        );
      },
    fetchMajors: (url) => 
      {
